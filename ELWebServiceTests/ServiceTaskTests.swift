...
extension ServiceTaskTests {
    ...
    func test_setJSON_encodesDataInRequestBody() {
        ...
        let bodyJSON = try? JSONSerialization.JSONObjectWithData(recordedURLRequest!.HTTPBody!, options: JSONSerialization.ReadingOptions())
...
extension ServiceTaskTests {
    class NonRespondingSession: Session {
        func dataTask(_ request: URLRequestEncodable, completion: (NSData?, URLResponse?, NSError?) -> Void) -> DataTask {
            return MockDataTask()
        }
    }
